...
class NewRecipePage extends React.Component<MyPropsInterface, IState> {
    ...
    static async getInitialProps({ query }: NextPageContext) {
        const recipe : ServerResponse = await axios.get(`http://localhost:5000/api/v1/Recipe/${query.id}`);
        return {
            recipe: recipe.data
        }
    }
    ...
    submitRecipe = () => {
        axios.patch(`/api/v1/Recipe/${this.state.recipe.recipeId}`, {
            ...this.state.recipe
        })
    }
            ...
            <div className="p-newrecipe">
                <Container>
                    ...
                    <div className="p-newrecipe__divider">
                        <Input name="recipe.name" onChange={this.fieldsOnChange} defaultValue={this.props.recipe.name} placeholder="Receptnamn..." />
                    </div>
                    <div className="p-newrecipe__divider">
                        <TextArea name="recipe.description" placeholder="Testdescription" defaultValue={this.props.recipe.description} autosize={{minRows: 15}} onChange={this.fieldsOnChange} />
                    </div>
                    <div className="p-newrecipe__divider">
                        <TextArea name="recipe.recipeIngredients" placeholder="Ingredienser"  defaultValue={this.props.recipe.recipeIngredients} autosize={{minRows: 15}} onChange={this.fieldsOnChange} />
                    </div>
                    <div className="p-newrecipe__divider">
                        <TextArea name="recipe.recipeInstructions" placeholder="Instruktioner" defaultValue={this.props.recipe.recipeInstructions} autosize={{minRows: 15}} onChange={this.fieldsOnChange} />
                    </div>
                    <div className="p-newrecipe__divider">
